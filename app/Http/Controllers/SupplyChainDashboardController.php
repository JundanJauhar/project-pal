...
class SupplyChainDashboardController extends Controller
{
    ...
    public function ajaxContract(Request $request)
    {
        try {
            ...
            // Format response - flatten kontrak per row
            $data = [];
            foreach ($procurements as $p) {
                foreach ($p->kontraks as $kontrak) {
            ...
            \Log::error('ajaxContract error: ' . $e->getMessage() . ' - ' . $e->getFile() . ':' . $e->getLine());
            return response()->json([
                'error' => $e->getMessage()
            ], 500);
        }
    ...
    public function ajaxPayment(Request $request)
    {
        try {
            ...
            $query = Procurement::with([
                ...
                'pembayarans' => function ($q) use ($typeFilter) {
                    $q->select(
                        ...
                        ->with('vendor:id_vendor,name_vendor')
                ...
                if ($procurement->pembayarans->isEmpty()) {
                    ...
                    foreach ($procurement->pembayarans as $payment) {
                        $data[] = [
                            ...
                            'payment_type' => $payment->payment_type ?? 'SKBDN',
                            'payment_value' => $payment->payment_value ?? 0,
                            'percentage' => $payment->percentage ?? 0,
                            'currency' => $payment->currency ?? 'IDR',
                            'no_memo' => $payment->no_memo ?? '-',
                            'link' => $payment->link ?? null,
                            'lsd' => optional($payment->lsd)->format('d/m/Y') ?? '-',
                            'evidence_link' => $payment->evidence_link ?? null,
                            'target_date' => optional($payment->target_date)->format('d/m/Y') ?? '-',
            ...
            \Log::error('ajaxPayment error: ' . $e->getMessage() . ' - ' . $e->getFile() . ':' . $e->getLine());
            return response()->json([
                'error' => $e->getMessage()
            ], 500);
        }
